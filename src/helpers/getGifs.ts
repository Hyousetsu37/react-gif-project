...
export const getGifs = async (category: string): Promise<Gifs[]> => {
  const searchString = category.includes(' ')
    ? category.split(' ').join('-')
    : category;
  const randomOffset = Math.floor(Math.random() * 37);
  const url = `https://api.giphy.com/v1/gifs/search?api_key=${
    import.meta.env.VITE_GIF_API_KEY
  }&q=${searchString}&limit=10&offset=${randomOffset}`;
  const resp = await fetch(url);
  const { data }: GIF = await resp.json();

  const gifs: Gifs[] = data.map((img) => ({
    id: img.id,
    title: img.title,
